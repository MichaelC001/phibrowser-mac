...
#format-field{position:absolute;left:120px;top:380px;width:280px;height:40px}
</style>
...
document.getElementById('format-field').addEventListener('input', (event) => {
...
async function main() {
  ...
  const formatted = await H.fillInput('#format-field', 'abc', { instant: true })
  const formatValue = await H.js('document.getElementById("format-field").value')
  check('setter fallback tolerates a field that reformats the value',
